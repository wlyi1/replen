...
def calculate_plano_target_gap(row):
    ...
    if not pd.notna(unit_qty) or not pd.notna(conversion_factor) or conversion_factor <= 0:
        return np.nan
    # plano_unit_qty is in ordering units (boxes), effective_qty is in sales units (strips).
    # Convert effective to ordering units first, then compute gap.
    # e.g. plano=3 boxes, effective=25 strips, factor=10 → CEIL(3 - 25/10) = CEIL(0.5) = 1 box
    effective_in_ordering = effective_qty / conversion_factor
    target_gap = np.ceil(unit_qty - effective_in_ordering)
...
def apply_hold_to_plano_rules(ordering_df, hold_to_plano_rules_dict):
    ...
    # plano_unit_qty is in ordering units (boxes); effective_qty is in sales units (strips).
    # Convert effective to ordering units, then compute the gap.
    # e.g. plano=3 boxes, effective=25 strips, factor=10 → CEIL(3 - 25/10) = 1 box
...
def apply_plano_stock_ratio_gate(ordering_df):
    """A third gate stacked on top of the two planogram mechanisms above
    (apply_hold_to_plano_rules's allowlist override, and the unconditional
    plano_unit_gap floor inside run_inventory_ordering/append_missing_plano_items),
    run right after both — so it sees their final ordered_qty_final — and still
    before allocate_tmp_stock().

    Even when the planogram floor or the hold_to_plano allowlist says a SKU
    should replenish, skip it anyway if the pharmacy already holds a healthy
    fraction of the planogram's own target — only let a planogram-driven order
    actually proceed into TMP allocation when current stock covers LESS than
    60% of the planogram target (stock/plano_qty ratio < 0.6). At or above that
    ratio the shelf isn't meaningfully under-stocked relative to its own
    target, so this forces the qty to 0 instead of letting it compete for
    (possibly scarce) central-warehouse stock over a SKU that's already
    reasonably covered.

    Only touches rows either planogram mechanism actually flagged
    (plano_unit_gap_applied OR hold_to_plano_applied) — a row neither
    mechanism touched is untouched by this gate too. Ported from the same
    rule added to the local dashboard's run_csv_replenish.py.
    """
    if ordering_df is None or ordering_df.empty:
        return ordering_df
    ...
    plano_qty = pd.to_numeric(result_df.get("plano_unit_qty"), errors="coerce")
    conversion_factor = pd.to_numeric(
        result_df.get("conversion_factor", 1.0), errors="coerce"
    ).fillna(1.0)
    effective_qty = pd.to_numeric(
        result_df.get("effective_qty", 0.0), errors="coerce"
    ).fillna(0.0)

    # plano_unit_qty is in ordering units (boxes); convert to sales units so it's
    # directly comparable to effective_qty (sales units) — same unit-matching
    # rule used everywhere else plano_unit_qty is compared against stock.
    ...
    result_df["plano_stock_ratio"] = ratio

    plano_touched = (
        result_df.get("plano_unit_gap_applied", False).fillna(False)
        | result_df.get("hold_to_plano_applied", False).fillna(False)
    )
    suppress = plano_touched & pd.notna(ratio) & (ratio >= 0.6)

    if suppress.any():
        result_df.loc[suppress, "ordered_qty_final"] = 0.0
        if "final_status" in result_df.columns:
            result_df["final_status"] = result_df["final_status"].astype(object)
            result_df.loc[suppress, "final_status"] = "No need to replenish"
        if {"price", "total_order_val"}.issubset(result_df.columns):
            result_df["total_order_val"] = (
                pd.to_numeric(result_df["ordered_qty_final"], errors="coerce").fillna(0)
                * pd.to_numeric(result_df["price"], errors="coerce").fillna(0)
...
def run_inventory_ordering(balancing_df, abc_xyz_df=None, targets_df=None, conversion_df=None, stock_tmp_df=None, price_df=None):
    ...
    if "plano_unit_qty" in merged_df.columns:
        ...
        # plano_unit_qty is in ordering units (boxes); effective_qty is in sales units (strips).
        # Convert effective to ordering units first, then compute the gap.
        # e.g. plano=3 boxes, effective=25 strips, factor=25 → CEIL(3 - 25/25) = CEIL(2) = 2
...
def append_missing_plano_items(ordering_df, plano_df, price_df=None, stock_tmp_df=None, stock_transit_df=None):
    ...
    if "ordered_qty_final" in added_rows.columns:
        # Do NOT fall back to raw plano_unit_qty when target_gap is NaN
        # (NaN means conversion_factor is missing — raw plano is in a different unit).
...
load_queries = {
    ...
    "conversion_df": f"SELECT * FROM `{DATASET}.ino_conversion_staging`",
   ...
   # "hold_df": "SELECT CAST(NULL AS STRING) AS sku, CAST(NULL AS STRING) AS type, CAST(NULL AS STRING) AS applied_to WHERE FALSE",
    "hold_to_plano_df": "SELECT * FROM `dw_swiperx_dev_waliyudin.hold_tab`", #gsheet_inofarma_control_center_hold_product_stock_out
    "config_df": "SELECT * FROM `dw_spreadsheet_prod.gsheet_inofarma_control_center_special_control`",
    "stock_tmp_df": f"SELECT * FROM `{DATASET}.tmp_stock_dev`",
    "store_df": f"SELECT * FROM `dw_spreadsheet_prod.gsheet_inofarma_control_center_store`",
    "additional_df": "SELECT * FROM `dw_spreadsheet_prod.gsheet_inofarma_control_center_additional_order`",
    "stock_transit_df": f"SELECT * FROM `{DATASET}.ino_stock_intransit_staging`",
...
# Third gate, still before TMP allocation: a row either planogram mechanism just touched
# (plano_unit_gap_applied or hold_to_plano_applied) only actually proceeds if current stock
# covers under 60% of the planogram's own target — see apply_plano_stock_ratio_gate()'s docstring.
ordering_df = run_timed("apply_plano_stock_ratio_gate", apply_plano_stock_ratio_gate, ordering_df)
