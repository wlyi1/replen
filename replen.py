...
def apply_hold_to_plano_rules(ordering_df, hold_to_plano_rules_dict):
    """Override forecast-based orders with the initial planogram quantity.
    ...
    for the matching SKU and pharmacy scope. For this special case, use
    plano_unit_qty directly without subtracting stock or applying a conversion.
    ...
    valid_plano = applicable & plano_qty.notna()
    missing_plano = applicable & plano_qty.isna()

    result_df.loc[valid_plano, "ordered_qty_final"] = plano_qty.loc[
        valid_plano
    ].clip(lower=0.0)
    ...
    if "final_status" in result_df.columns:
        ...
        positive_plano = valid_plano & plano_qty.gt(0)
        zero_plano = valid_plano & plano_qty.le(0)
        result_df.loc[positive_plano, "final_status"] = (
            plano_qty.loc[positive_plano].astype(float).astype(str)
        )
        result_df.loc[zero_plano, "final_status"] = "No need to replenish"
    ...
    result_df.loc[missing_plano, "ordered_qty_final"] = 0.0
    result_df.loc[missing_plano, "hold_to_plano_applied"] = True
...
def append_missing_plano_items(ordering_df, plano_df, price_df=None, stock_tmp_df=None, stock_transit_df=None):
    ...
    if "ordered_qty_final" in added_rows.columns:
        added_rows["ordered_qty_final"] = added_rows["target_gap"].fillna(added_rows["plano_unit_qty"])
    if "final_status" in added_rows.columns:
        added_rows["final_status"] = added_rows["target_gap"].fillna(added_rows["plano_unit_qty"])
...
load_queries = {
    ...
    "hold_to_plano_df": "SELECT * FROM `dw_spreadsheet_prod.gsheet_inofarma_control_center_hold_to_plano`",
