...
def run_inventory_ordering(balancing_df, abc_xyz_df=None, targets_df=None, conversion_df=None, stock_tmp_df=None, price_df=None):
    ...
    if "plano_unit_qty" in merged_df.columns:
        ...
        # Subtract in sales units first, then convert to ordering units.
        # e.g. plano=4, effective=3, factor=10 → gap_sales=1 → CEIL(1/10)=1
        gap_in_sales = np.maximum(unit_qty.loc[valid_gap] - effective.loc[valid_gap], 0.0)
        gaps = np.ceil(gap_in_sales / factor.loc[valid_gap])
...
load_queries = {
    ...
    "conversion_df": f"SELECT * FROM `{DATASET}.ino_conversion`",
